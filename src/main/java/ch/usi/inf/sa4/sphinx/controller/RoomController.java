...
public class RoomController {
    ...
    public ResponseEntity<Collection<SerialisableDevice>> getDevice(@PathVariable final Integer roomId,
        ...
        final User user = userService.get(username).orElseThrow(()->new ServerErrorException(""));//It exists from previous check
        final Room room = roomService.get(roomId).orElseThrow(() -> new ServerErrorException(""));//It exists from previous check
    ...
    public ResponseEntity<SerialisableRoom> createRoom(@NotBlank @RequestHeader("session-token") final String sessionToken,
        ...
        final Integer id = userService.addRoom(username, room).orElseThrow(() -> new ServerErrorException(""));
    ...
    public ResponseEntity<SerialisableRoom> modifyRoom(@NotBlank @PathVariable final Integer roomId,
        ...
        final Room storageRoom = roomService.get(roomId).orElseThrow(() -> new ServerErrorException(""));
    ...
    public ResponseEntity<SerialisableRoom> deleteRoom(@NotBlank @PathVariable final Integer roomId,
                                                       @NotBlank @RequestHeader("session-token") final String sessionToken,
                                                       @NotBlank @RequestHeader("user") final String username) {
        check(sessionToken, username, null, roomId);
